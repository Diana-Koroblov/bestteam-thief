...
**Read the pattern before the list.** Nine defects, and 1588 tests at 95 % coverage had caught none of them: every one lives where the code meets something the suite is not allowed to touch — a real GPU, a real ngrok account, a real mailbox, two real processes writing one file, and a human reading a document. Coverage measures which lines ran, not which of them ran against reality.
