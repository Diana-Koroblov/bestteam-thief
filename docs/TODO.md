...
- [x] 7.2.1 [D] - `declaration_<game_id>.json` builder | DoD: Teams, members, **four** repo links, MCP URLs, hardware, LLM model, token cap, timings. — `core/report/artefacts.py`.
  ...
  - [ ] 7.2.1.d [B] - ⛔ **`repositories` is still four empty strings.** `MatchFiling.repos` is never populated in production, so both the declaration and the emailed result file `{"ours_cop": "", ...}` — against Ch. 9.4, which requires four links in the closing JSON (M#49). The URLs are prose in the README and the handshake never exchanges them. Needs: our two in `[identity]`, theirs in the negotiation payload.
