...
- [ ] 7.QG.4 🧑 [B] - **Milestone M7 observed** — **1 of 3 done, 05/08** | DoD: Summary reaches the inbox; GUI shows live state; Replay reproduces a recorded series with `Verified OK`. **The only part of Phase 7 code cannot finish.** Three human steps, none blocked by anything but a person:
  ...
  - [x] **Screenshot the Live GUI during a live match** — `uv run python -m core play --role cop --gui ...`. **Captured 13/08, `docs/evidence/m7-live-gui-belief.png`.** Step 23 of a real sub-game over MCP: a deep-red maximum with graded neighbours, the cop's own marker, the green turn banner and the hint just received. This is the belief map Ch. 9.4 asks for and closes the ⚠️ below, which stood from 05/08 because the only obtainable capture was step 0.
...
- [x] 9.6.13 [D] - 🐛 **The Live GUI could not watch a match, and the belief-map capture is an absolute requirement.** `--gui` was a flag on the `peer` subcommand and a **dead end** there: it opened the window and returned without playing, so the choice was a board that never moved or a match with nothing to watch. `play` had no such flag at all. Ch. 7.3 has each side running its software out of a dedicated GUI; Ch. 9.4 makes a screen capture of the belief map an absolute submission requirement; the definition of done says the viewing tools display the game **in real time**. The only capture obtainable was step 0 — a uniform prior, every cell at peak intensity, a flat wash of red that shows a working renderer and nothing about belief, which is exactly what the ⚠️ under 7.4.1's evidence line had said since 05/08. Now `core/cli_gui.py` hosts the window on the main thread (Tk must own the one it was created on) with the match on a worker. Three properties are tested rather than trusted: the exit code is the match's, so `--gui` changes what is on screen and nothing about what the command means; **closing the window never forfeits** — the worker is not a daemon and is joined, because a close button that abandoned a series mid-sub-game would be an M#35 trap worth 0 to both teams; and a display that cannot open at all is reported and ignored, since the match is the deliverable. Captured live at step 23: `docs/evidence/m7-live-gui-belief.png`.
