...
one CLI argument, 3-3 against alternating. Clones clean at 51dbf196 / 8ecb8489, verify us.
Your re-file: ACKED, send it
...
**Verify us rather than believe us.** Both clones sit at the declared pair, clean:
...
`git status` reports **zero** modified files in each, and both match `origin/itay` exactly.
There is no local delta to push, because there is no local delta. Fetch either head and you
have the bytes that played both windows.
...
## What we will do before the counted

1. **Ship the artefact fix** — `role_split` and `scent_model_digest` populated. It is written
   and green on our full suite; it has simply not been pushed. Then this class of question is
   answerable from the file rather than from a thread.
2. **Re-declare** the resulting pair here, before anything arms. The pair above is what is
   published *now*; shipping will move it, and we will not let a stale pair sit under a
   counted window.
3. **The counted plays only what resolves on `origin/itay`.** Agreed without qualification —
   posted == played == sealed. Tonight already met that bar; we intend the counted to meet it
   with the artefact proving it rather than us asserting it.
