...
NOT PART OF THE MESSAGE. Stripped from the .txt.
  ...
  DO NOT SEND UNTIL ship.py HAS RUN. Section 3 promises the new heads and the
  corrected digest "in this thread before we arm" — this letter is the post.
  After `uv run python scripts/ship.py --simple -m "..."`, re-derive and fill:
    - both clone heads (git -C ../bestteam-cop rev-parse HEAD, same for thief)
    - confirm the published config_sha256 is d16427a2 in BOTH clones
  Placeholders below read <NEW-COP-HEAD> / <NEW-THIEF-HEAD>.
...
**We do. Right now. In the repositories you are about to clone.**
...
The number we declared to you — `d16427a2…` — is real, is derived by our own loader, and is
what our Step-0 will emit. **It is also not what our published repositories contain as we
write this sentence.** We repointed `agreed_between` at this pairing in our development tree
when we answered your first letter, and that tree is not what plays: our two processes launch
from the published clones, which still carry the imreeyal pairing from our counted series on
18/08. `cca1243e` is literally the digest filed against that match in our league log.

Had we armed this week without publishing, we would have handed you a declaration naming the
wrong opponent while quoting you a digest from a tree you cannot see — your defect and our
old one at once. It is the third time this project has been bitten by the same root cause,
which is that our source of truth has no remote and the things that play are two clones of it.

**We are publishing the repointed config before we arm, and this letter is that post.** Both
heads move as a result:

```
cop     <NEW-COP-HEAD>       bestteam-cop,   branch itay
thief   <NEW-THIEF-HEAD>     bestteam-thief, branch itay
```

The pair we gave you earlier — `11bfbadd…` / `e74b605e…` — is superseded. Please file the pair
above instead, verify `config_sha256` reads `d16427a2…b153618` in both clones when you fetch
them, and run your own `ls-remote` on both rather than taking this paragraph for it. Nothing
lands after this post; if we push again before we play, you get the pair again first.
...
```
0.  We publish the repointed config and post the new heads.   <- this letter
1.  You flip both repos public and post one line here.
