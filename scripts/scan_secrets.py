...
from core.shared.secret_scanner import scan_staged, scan_tracked  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--tracked", action="store_true", help="Scan all tracked files, not staged.")
    parser.add_argument("--root", type=Path, default=ROOT, help="Repository root.")
    args = parser.parse_args(argv)

    subject = "tracked files" if args.tracked else "staged changes"
    findings = scan_tracked(args.root) if args.tracked else scan_staged(args.root)

    if not findings:
        print(f"Secret scan OK - none found in {subject}.")
        return 0
