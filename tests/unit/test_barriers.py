...
def test_quota_comes_from_config(barrier_manager: BarrierManager) -> None:
    assert barrier_manager.remaining == 14
    assert barrier_manager.placed_count == 0
...
def test_own_cell_and_four_orthogonal_neighbours_are_legal(
    barrier_manager: BarrierManager, target: tuple[int, int]
) -> None:
    assert barrier_manager.can_place(target, COP, True)
...
def test_diagonally_adjacent_is_rejected(
    ...
    result = barrier_manager.place(target, COP)
...
def test_anything_further_than_one_step_is_rejected(
    ...
    assert not barrier_manager.can_place(target, COP, True)


def test_a_target_off_the_board_is_rejected(barrier_manager: BarrierManager) -> None:
    result = barrier_manager.place((-1, 0), (0, 0))
    assert result.reason is RejectionReason.OFF_BOARD


def test_placing_while_moving_is_rejected(barrier_manager: BarrierManager) -> None:
    """T1.9: the barrier costs the Cop its move for that turn."""
    result = barrier_manager.place((2, 3), COP, is_forgoing_move=False)
    assert result.outcome is PlacementOutcome.REJECTED
    assert result.reason is RejectionReason.NOT_FORGOING_MOVE
    assert barrier_manager.remaining == 14


def test_the_same_cell_cannot_be_blocked_twice(barrier_manager: BarrierManager) -> None:
    assert barrier_manager.place((2, 3), COP).succeeded
    result = barrier_manager.place((2, 3), COP)
    assert result.reason is RejectionReason.ALREADY_BLOCKED
    assert barrier_manager.placed_count == 1
...
def test_fourteen_placements_are_accepted_and_the_fifteenth_is_not() -> None:
    ...
    barrier_manager = BarrierManager(max_barriers=14, board=BOARD)
    cells = [(row, col) for row in range(2) for col in range(7)]
    for index, cell in enumerate(cells):
        assert barrier_manager.place(cell, cell).succeeded, index
    assert barrier_manager.placed_count == 14
    assert barrier_manager.remaining == 0

    fifteenth = barrier_manager.place((3, 0), (3, 0))
    assert fifteenth.outcome is PlacementOutcome.REJECTED
    assert fifteenth.reason is RejectionReason.QUOTA_EXHAUSTED
    assert barrier_manager.placed_count == 14


def test_a_rejected_placement_never_costs_quota(barrier_manager: BarrierManager) -> None:
    barrier_manager.place((0, 0), COP)  # out of range
    barrier_manager.place((2, 3), COP, is_forgoing_move=False)  # not forgoing
    assert barrier_manager.remaining == 14
...
def test_there_is_no_way_to_remove_a_barrier(barrier_manager: BarrierManager) -> None:
    """1.3.1.d: permanence is enforced by the absence of an API, not a flag."""
    for name in ("remove", "clear", "reset", "undo", "delete", "pop"):
        assert not hasattr(barrier_manager, name), name


def test_the_exposed_barrier_set_is_frozen(barrier_manager: BarrierManager) -> None:
    barrier_manager.place((2, 3), COP)
    assert isinstance(barrier_manager.barriers, frozenset)
...
def test_a_placed_barrier_stays_for_the_rest_of_the_sub_game(
    ...
    barrier_manager.place((2, 3), COP)
    for _ in range(5):
        barrier_manager.place((3, 4), COP)
    assert (2, 3) in barrier_manager.barriers


# --- captures ---------------------------------------------------------------


def test_placing_on_the_thief_captures(barrier_manager: BarrierManager) -> None:
    """T1.10, M#46."""
    result = barrier_manager.place((2, 3), COP, thief_pos=(2, 3))
    assert result.outcome is PlacementOutcome.CAPTURE
    assert result.cell == (2, 3)


def test_sealing_the_last_exit_captures(barrier_manager: BarrierManager) -> None:
    """M#47: the Thief loses without anyone moving onto it."""
    barrier_manager = BarrierManager(max_barriers=14, board=BOARD)
    thief = (0, 0)
    assert barrier_manager.place((1, 0), (1, 0), thief_pos=thief).outcome is PlacementOutcome.PLACED
    final = barrier_manager.place((0, 1), (0, 1), thief_pos=thief)
...
def test_a_placement_that_leaves_an_exit_does_not_capture(
    ...
    assert barrier_manager.place((2, 3), COP, thief_pos=(3, 3)).outcome is PlacementOutcome.PLACED
...
def test_capture_is_not_reported_when_the_thief_is_unobserved(
    ...
    assert barrier_manager.place((2, 3), COP).outcome is PlacementOutcome.PLACED
...
def test_a_rejected_placement_reports_the_cell_it_was_asked_about(
    ...
    result = barrier_manager.place((0, 0), COP)
