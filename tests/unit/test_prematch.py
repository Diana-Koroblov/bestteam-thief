...
def test_a_contract_naming_both_teams_is_not_flagged(minimal_config, league) -> None:
    """Once both are named the warning must go, or it becomes noise to scroll past."""
    from dataclasses import replace as replace_field

    signed = replace_field(
        minimal_config, shared={**minimal_config.shared, "agreed_between": ["bestteam", "them"]}
    )
    peer = PreMatch(
        orchestrator=Orchestrator.from_config(signed, Role.COP), league_log_path=league
    )
    assert not any("agreed_between" in warning for warning in peer.warnings())
